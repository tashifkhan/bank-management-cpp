...
void new_acc()
{
    ...
    while (fscanf(ptr, "%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d\n", &add.acc_no, add.name, &add.dob.day, &add.dob.month, &add.dob.year, &add.age, add.address, add.aadhar, &add.phone, add.acc_type, &add.amt, &add.deposit.month, &add.deposit.day, &add.deposit.year) != EOF)
    {
        if (check.acc_no == add.acc_no)
        {
            printf("Account no. already in use!");
            fordelay(1000000000);
            // delay geerate hua loop bhot lamba chala 
            goto account_no;
        }
    }
...
void view_list()
{
    ...
    while (fscanf(view, "%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d", &add.acc_no, add.name, &add.dob.day, &add.dob.month, &add.dob.year, &add.age, add.address, add.aadhar, &add.phone, add.acc_type, &add.amt, &add.deposit.month, &add.deposit.day, &add.deposit.year) != EOF)
    {
...
void close()
{
    printf("\n\n\n\nThe END...\n\n\n~project developed by TASHIF AHMAD KHAN & SHASHWAT SINGH.");
}
...
int main()
{
    char pass[30], password[30] = "who_are_you? hun? CHEETOS";
    int i = 0;
